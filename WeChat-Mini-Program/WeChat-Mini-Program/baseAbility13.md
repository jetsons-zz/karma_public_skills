...
使用小程序提供的 [rpx](https://developers.weixin.qq.com/miniprogram/dev/framework/view/wxss.html#尺寸单位) 单位，在尺寸差别不大的情况下对页面布局进行等比缩放。
...
小程序基础库基于 `window.matchMedia API` 新增了一组过程式与定义式接口 `match-media` 。开发者可以通过 [``](https://developers.weixin.qq.com/miniprogram/dev/component/match-media.html) 和 [`wx.createMediaQueryObserver`](https://developers.weixin.qq.com/miniprogram/dev/framework/ability/(wx.createMediaQueryObserver)) 来显式地使用媒体查询能力，对于多端适配来说，它有以下优势：
...
![img](https://res.wx.qq.com/wxdoc/dist/assets/img/adapt-4.29662d8d.png)

示例小程序中，基于 [row/col 组件](https://developers.weixin.qq.com/miniprogram/dev/extended/component-plus/grid.html) 简单实现了常见的适配场景，例如：

- 屏幕越大，布局不变，模块左右伸缩

![img](https://res.wx.qq.com/wxdoc/dist/assets/img/adapt-1.908bda6e.png)

- 屏幕越大，内容越多，模块内容换行排列

![img](https://res.wx.qq.com/wxdoc/dist/assets/img/adapt-2.6e1e7186.png)

- 屏幕越大，布局改变，模块内容可折叠 / 展现

![img](https://res.wx.qq.com/wxdoc/dist/assets/img/adapt-3.ef046dab.png)

## 4. 分栏模式

在 PC 等能够以较大屏幕显示小程序的环境下，小程序支持以[分栏模式](https://developers.weixin.qq.com/miniprogram/dev/framework/view/frameset.html)展示。分栏模式可以将微信窗口分为左右两半，各展示一个页面。

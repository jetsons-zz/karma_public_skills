...
- 默认情况下，仅能在这个分包内使用该插件，除非通过 [分包异步化](https://developers.weixin.qq.com/miniprogram/dev/framework/subpackages/async.html) 进行异步的跨分包引用；
- 同一个插件不能被多个分包同时引用；
...
- 默认情况下，小程序和其他插件的
  ...
  - 这个限制可以通过 [自定义的组件实例获取结果](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/events.html#自定义的组件实例获取结果) 来去除
...
基础库 [2.14.0](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 起，也可以通过插件的 AppID 来获取接口，如：
...
从基础库 [2.11.1](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 起，使用插件的小程序可以导出一些内容，供插件获取。具体来说，在声明使用插件时，可以通过 `export` 字段来指定一个文件，如：
...
另外也可以 [参考开发插件的相关文档](https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/development.html#获取小程序导出)

### 为插件提供自定义组件

> [在开发者工具中预览效果](https://developers.weixin.qq.com/s/QRRovLmu7Xjm)，需要手动填写一下 `miniprogram/app.json` 中的插件 AppID

有时，插件可能会在页面或者自定义组件中，将一部分区域交给使用的小程序来渲染，因此需要使用的小程序提供一个自定义组件。但由于插件中不能直接指定小程序的自定义组件路径，因此需要通过为插件指定 [抽象节点（generics）](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/generics.html) 的方式来提供。
...
从基础库 [2.12.2](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 起，可以通过配置项为插件页面指定抽象组件实现。例如，要给插件名为 `plugin-index` 的页面中的抽象节点 `mp-view` 指定小程序的自定义组件 `components/comp-from-miniprogram` 作为实现的话：
...
另外也可以 [参考开发插件的相关文档](https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/development.html#引用小程序的自定义组件)
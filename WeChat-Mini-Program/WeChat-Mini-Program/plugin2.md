...
![创建插件](https://res.wx.qq.com/wxdoc/dist/assets/img/createplugin.50189a78.png)
...
插件从小程序基础库版本 [2.1.0](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 开始支持页面。插件可以定义若干个插件页面，可以从本插件的自定义组件、其他页面中跳转，或从使用者小程序中跳转。所有页面必须在配置文件的 `pages` 段中列出（参考上文）。
...
从基础库 [2.11.1](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 起，在插件中有全局函数 `requireMiniProgram`，可以获取由使用者小程序导出的内容。
...
另外也可以 [参考使用者小程序的相关文档](https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/using.html#导出到插件)

### 引用小程序的自定义组件

> [在开发者工具中预览效果](https://developers.weixin.qq.com/s/QRRovLmu7Xjm)，需要手动填写一下 `miniprogram/app.json` 中的插件 AppID

有时，插件可能需要在页面或者自定义组件中，将一部分区域交给使用的小程序来渲染，因此需要能够引用小程序的自定义组件。但由于插件中不能直接指定小程序的自定义组件路径，因此无法直接通过 `usingComponents` 的方式来引用。这里介绍通过 [抽象节点（generics）](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/generics.html) 来实现引用的方式。
...
如果是插件页，插件页本身就是一个页面顶层组件，小程序不会引用它，无法通过 `generic:xxx=""` 的方式来指定抽象节点实现；因此，从基础库 [2.12.2](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html) 起，小程序可以在插件的配置里为插件页指定抽象节点实现。例如插件页面名为 `plugin-index`，则可以：
...
另外也可以 [参考使用者小程序的相关文档](https://developers.weixin.qq.com/miniprogram/dev/framework/plugin/using.html#为插件提供自定义组件)
